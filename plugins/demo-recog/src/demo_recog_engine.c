...
void release_string(struct res_string *s)
{
  free(s->ptr);
  return;
}
...
size_t writefunc(void *ptr, size_t size, size_t nmemb, struct res_string *s)
{
 ...
 int speech2txt(char   *path,  struct res_string *s){
  CURL *curl;  
  CURLcode res;
...
static apt_bool_t demo_recog_result_load(demo_recog_channel_t *recog_channel, mrcp_message_t *message)
{
  ...
  init_string(&res);
  // char *res;
  speech2txt(file_path,&res);
  printf("%s\n",res.ptr );
  apt_string_assign_n(&message->body,res.ptr,res.len,message->pool);
